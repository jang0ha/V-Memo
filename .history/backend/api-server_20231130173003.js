...
// ------------- GET ------------- //
app.get('/api/memos', async (req, res) => {//2.url변경 api 꼭 붙히기 [get  데이터 조회]
...
app.put('/api/memos/:id', async(req, res) => { // :idx값넣기 FormList.vue의 eidt() + idx 를 :idx로 표현
	...
	await database.run(`UPDATE memos SET content = '${req.body.content}' WHERE id = ${req.params.id}`);//12. 데이터 수정
	const result = await database.run('SELECT * FROM memos');
